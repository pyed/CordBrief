...
func runDigest(args []string, stdout, stderr io.Writer) int {
	...
	if *cfgPath != "" {
		...
		} else if !errors.Is(err, os.ErrNotExist) || *cfgPath != "config.json" {
...
func printUsage(w io.Writer) {
	...
	fmt.Fprintln(w, "  serve       Run the Web inbox and scheduler")
	fmt.Fprintln(w, "  migrate     Add durable citation sources to older digest artifacts")
