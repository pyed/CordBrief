...
// Backed by an advisory flock on <dataDir>/commit.lock which is automatically released
// by the kernel upon process termination or crash.
...
// AcquireCommitLock attempts to acquire exclusive committing ownership via flock on <dataDir>/commit.lock.
